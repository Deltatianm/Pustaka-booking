...
{ 
    ...
    { 
        ...
        if ($databooking > 0) { 
            $this->session->set_flashdata('pesan', '<div class="alert alert
danger alert
message" role="alert">Masih Ada booking buku sebelumnya yang belum diambil.<br> A
 ...
 ; 
        ...
        if ($temp > 0) { 
            $this->session->set_flashdata('pesan', '<div class="alert alert
danger alert-message" role="alert">Buku ini Sudah anda booking </div>'); 
...
message" role="alert">Buku berhasil ditambahkan ke keranjang </div>'); 
    ...
    { 
        ...
        $data['items'] = $this->db->query("select*from booking bo, booking_detail d, buku bu where          
         d.id_booking=bo.id_booking and d.id_buku=bu.id and bo.id_user='$where'")->result_array(); 
    ...
    { 
        ...
        $data['useraktif'] = $this->ModelUser->cekData(['id' => $this->session->userdata('id_user')])->result(); 
        $data['items'] = $this->db->query("select*from booking bo, booking_detail d, buku bu where 
         d.id_booking=bo.id_booking and d.id_buku=bu.id and bo.id_user='$id_user'")->result_array(); 
        ...
        //$this->dompdf->render(); 
        $this->pdf->filename="bukti-booking-$id_user.pdf";//, array('Attachment' => 0); 
